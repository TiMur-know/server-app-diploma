...
const calculateBaseTicketPrice=(distance, fuelPricePerKm, companyProfit, additionalExpenses)=> {
...
const getRoutesWithTransportName = async (filter = {}) => {
	try {
			...
			const routesWithDetails = dataJson.routes.map(route => {
					const transport = dataJson.transports.find(t => t.id === route.transport_id);
					return {
...
const calculateAll = async () => {
	try {
			...
			for (let i = 0; i < routesData.length; i++) {
					...
					const transport = parsedBusData.transports.find(t => t.id === route.transport_id);

					if (transport) {
							const fuelPricePerKm = calculateFuelPricePerKm(transport.fuel_consumption, route.fuel_price_per_litter);
							const baseTicketPrice = calculateBaseTicketPrice(distance, fuelPricePerKm, transport.company_tax, transport.maintenance_cost);
...
module.exports={
	getRoutes,addRoute,updateRoute,deleteRoute,calculateAll,getRoutesWithTransportName
}
